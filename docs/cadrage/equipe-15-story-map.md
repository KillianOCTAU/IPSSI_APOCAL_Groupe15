...
| Niveau MoSCoW | 1. S'inscrire | 2. Uploader un cours | 3. Générer un quiz | 4. Passer le quiz | 5. Consulter résultats | 6. Gérer son compte (RGPD) | 7. Suivre sa classe (enseignant) |
|---|---|---|---|---|---|---|---|
| **MUST - MVP Release 1** *(Sprint 1–5)* | **US-01** F1 - Inscription email + mot de passe (Django Auth standard) | **US-02** F2 - Upload PDF ≤ 5 Mo OU saisie texte ≥ 200 caractères | **US-03** F3 - Génération automatique de 10 QCM via Llama 3.1 8B (Ollama local) | **US-04** F4 - Soumission + correction automatique (1 bonne réponse / QCM) | **US-05** F5 - Score /10 + détail des bonnes et mauvaises réponses | **US-06** F6 - Historique persistant des quiz par utilisateur (date, cours, score) | **US-25** † Dashboard classe : scores + taux complétion des 28 étudiants · **US-26** † Alertes décrocheurs (score moyen < 5/10 sur 3 derniers quiz) |
| **SHOULD - Release 2** *(Sprint 6–7)* | **US-07** Reset password par email (lien magique valide 24 h) | **US-08** Multi-cours : bibliothèque personnelle de cours uploadés | **US-09** ★ Choix du niveau de difficulté + nombre de questions (5–20) - *Vision Board candidat 2* | **US-10** Mode timer optionnel par question (10–30 s configurable) | **US-11** ★ Explication de la bonne réponse après correction - *Vision Board candidat 1* | **US-12** Export RGPD complet (JSON + CSV, Art. 15/20) - *lié perturbation J3-bis* | **US-27** † Envoyer un conseil à un étudiant depuis son profil |
| **COULD - Release 2 si temps** *(Sprint 7 ou backlog futur)* | **US-13** Login social Google / Apple OAuth | **US-14** ★ Partage d'un quiz par lien public (sans compte obligatoire) - *Vision Board bonus* | **US-15** Mode questions ouvertes (correction LLM avec barème) | **US-16** Mode flashcards type Anki *(→ R2, sorti de R1 - trade-off perturbation J1)* | **US-17** Identification automatique des lacunes par chapitre | **US-18** Suppression compte + données (RGPD Art. 17, droit à l'oubli) | **US-28** † Historique quiz par étudiant de la classe |
| **WON'T (this time)** *(décision explicite - à documenter)* | **US-19** SSO entreprise SAML / OIDC (B2B Release 3+) | **US-20** Source vidéo ou audio (transcription Whisper - coût/complexité prohibitif) | **US-21** Compétition multi-joueurs (hors cible primaire Léa) | **US-22** Mode IA conversationnelle type chatbot (hors scope Khanmigo) | **US-23** Stats temps réel collectives (besoin marginal MVP) | **US-24** Personnalisation thème UI (thème sombre déjà livré dans le kit) | **US-29** Notifications push automatiques (hors scope semaine) |

> ★ = story alignée avec les candidats Release 2 sélectionnés dans l'Artefact 1 (Vision Board §4.3)
> † = nouvelle story enseignant ajoutée suite perturbation J1 (Lundi 14h00) · Persona co-primaire 2 : Mme Lefèvre
...
| US-25 † | MUST → Journey Map Mme Lefèvre étape 4 (dashboard classe) · Perturbation J1 | `backend/accounts/models.py` · champ `role` + entité `Classe` (US-T01) |
| US-26 † | MUST → Journey Map Mme Lefèvre étape 4 (alertes décrocheurs) · Perturbation J1 | Dépend de US-25 · logique agrégation scores `backend/quizzes/` |
| US-27 † | SHOULD → Journey Map Mme Lefèvre étape 4 (conseils étudiants) · Perturbation J1 | Nouveau endpoint `POST /api/messages/` à créer |
| US-28 † | COULD → Journey Map Mme Lefèvre étape 4 (historique par étudiant) · Perturbation J1 | `backend/quizzes/views.py` · filtre par étudiant + classe |
| US-T01 | Dette technique Sprint 1 - prérequis bloquant US-25/26/27/28 | Migration Django : modèle `Classe` + `User.role` |
