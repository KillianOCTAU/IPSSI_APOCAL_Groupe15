...
**Date** : 30/06/2026 (cadrage matinal)
**Auteurs** : Équipe 15
...
Lors du cadrage matinal du Sprint 0 (lundi 30/06/2026), l'équipe a dû choisir un modèle LLM
pour la génération de quiz. La contrainte stack imposée est Ollama local uniquement
(pas d'API externe, conformité RGPD, gratuité). Le kit APOCAL'IPSSI propose `llama3.1:8b`
comme modèle par défaut dans `.env.example`.
...
⚠️ Négatives : latence CPU-only élevée (~42 s p50) — acceptable en cadrage,
